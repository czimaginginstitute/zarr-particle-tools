...
def get_particle_crop_and_visibility(particle_id: int, sections: dict, tiltseries_x: int, tiltseries_y: int, tiltseries_pixel_size: float, box_size: int) -> tuple[list[dict], list[int]]:
    """
    ...
    for section, coords in sections.items():
        ...
        y_start_px = int(round(y_px_float - box_size / 2.0))
...
def shift_slice_rfft(spectrum: np.ndarray,
                     shift: tuple[float, float],   # (tx, ty) in pixels
                     out: np.ndarray | None = None
                    ) -> np.ndarray:
    """
    Multiply an rFFT half-spectrum by a phase ramp so that its corresponding
    real-space image is shifted by (tx, ty) pixels.

    Parameters
    ----------
    spectrum : (h, wh) complex ndarray
        Output of `np.fft.rfft2(real_image)`, where 'wh' = w//2 + 1.
    shift : (tx, ty) tuple
        Desired translation in pixels.  +tx → right, +ty → down.
    out : ndarray, optional
        Destination buffer.  Must have the same shape/dtype as `spectrum`.

    Returns
    -------
    ndarray
        The shifted spectrum (`out` if supplied, otherwise a new array).
    """
    # geometry
    h, wh = spectrum.shape
    w = (wh - 1) * 2                     # full real‑space width

    tx, ty = map(float, shift)

    # coordinate vectors
    x = np.arange(wh, dtype=np.float32)          # 0 … wh‑1
    y = np.arange(h,  dtype=np.float32)
    y[h//2:] -= h                                # 0 … h/2‑1, −h/2 … −1

    # phase ramp
    phi = (2*np.pi/w)*tx * x[None, :] + (2*np.pi/h)*ty * y[:, None]   # (h, wh)
    ramp = np.exp(-1j * phi)

    if out is None:
        out = np.empty_like(spectrum, dtype=np.complex64)

    np.multiply(ramp, spectrum, out=out)         # out = e^{iφ} * spectrum
    return out
...
def process_tiltseries(args) -> Union[None, tuple[pd.DataFrame, int]]:
    ...
    for particle_id, sections in particles_to_tiltseries_coordinates.items():
        ...
        for tilt in range(len(particle_data)):
            ...
            current_tilt = data[section - 1, y_start_px:y_end_px, x_start_px:x_end_px].copy().astype(np.float32)
            pyfftw_real = pyfftw.empty_aligned(current_tilt.shape, dtype=np.float32)
            fourier_tilt = pyfftw.empty_aligned((current_tilt.shape[0], current_tilt.shape[1] // 2 + 1), dtype=np.complex64)
            np.copyto(pyfftw_real, current_tilt)
            fft_object = pyfftw.FFTW(
                pyfftw_real, fourier_tilt,
                axes=(0, 1),
            ...
            fft_object()
            # scale = np.sqrt(current_tilt.shape[0] * current_tilt.shape[1])
            # fourier_tilt /= scale

            pyfftw_fourier_tilt = pyfftw.empty_aligned(fourier_tilt.shape, dtype=np.complex64)
            current_tilt_final = pyfftw.empty_aligned(current_tilt.shape, dtype=np.float32)
            np.copyto(pyfftw_fourier_tilt, fourier_tilt)
            fft_object = pyfftw.FFTW(
                pyfftw_fourier_tilt, current_tilt_final,
                axes=(0, 1),
            ...
            fft_object()
            # current_tilt_final /= scale
            current_tilt_final *= -1

            tilt_data[tilt] = current_tilt_final

            # apply subpixel shift
            # fourier_tilt = pyfftw.interfaces.dask_fft.fft(current_tilt).astype(np.complex64)
            # fourier_tilt = fourier_shift(fourier_tilt, subpixel_shift).astype(np.complex64)
            # fourier_tilt = shift_slice_rfft(fourier_tilt, subpixel_shift).astype(np.complex64)
            # fourier_tilt = np.fft.rfft2(current_tilt).astype(np.complex64)
...
def write_starfiles(merged_particles_df: pd.DataFrame, particle_optics_df: pd.DataFrame, tiltseries_starfile: str, box_size: int, bin: int, output_dir: str) -> None:
    """
